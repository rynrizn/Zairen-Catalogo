...
function renderSectionView(container) {
    ...
    const visibleProds = allProductos.filter(p => p.visible !== false);
    const sortedProds = getSortedProducts(visibleProds);
...
function renderFilteredView(container) {
    ...
    if (activeFilter === 'favoritos') {
        filtered = visibleProds.filter(p => favorites.includes(p.id));
    } else if (activeFilter.startsWith('tipo:')) {
        const tipo = activeFilter.replace('tipo:', '');
        filtered = visibleProds.filter(p => p.tipo === tipo);
    } else if (activeFilter.startsWith('estado:')) {
        const estado = activeFilter.replace('estado:', '');
        filtered = visibleProds.filter(p => p.estado === estado);
    } else {
        filtered = visibleProds;
    }
...
function createProductCard(producto, animIndex) {
    ...
    if (cc.whatsapp && cc.instagram) {
        if (cc.whatsapp.enabled && cc.instagram.enabled) {
            activeChannel = cc.last_active || "whatsapp";
        } else if (cc.whatsapp.enabled) {
            activeChannel = "whatsapp";
        } else if (cc.instagram.enabled) {
    ...
    if (hasMultipleImages) {
        carruselHTML = `
            <div class="carrusel-track" style="position: absolute; inset: 0; width: 100%; height: 100%;">
                ${imagenes.map((img, idx) => `
                    <img src="${img}" alt="${escNombre}" loading="lazy" class="card-img-element carrusel-slide ${idx === 0 ? 'active' : ''}" data-index="${idx}" style="
            ...
            <button type="button" class="carrusel-arrow prev" style="
            ...
            <button type="button" class="carrusel-arrow next" style="
            ...
            <div class="carrusel-dots" style="
                position: absolute; bottom: 8px; left: 50%; transform: translateX(-50%);
                display: flex; gap: 4px; z-index: 15;
            }">
                ${imagenes.map((_, idx) => `
                    <span class="carrusel-dot ${idx === 0 ? 'active' : ''}" data-index="${idx}" style="
                        width: 6px; height: 6px; border-radius: 50%; background: ${idx === 0 ? 'var(--z-crimson)' : 'rgba(255,255,255,0.4)'};
        ...
        carruselHTML = `<img src="${producto.imagen || (imagenes[0] || '')}" alt="${escNombre}" loading="lazy" class="card-img-element">`;
    }
    ...
    card.innerHTML = `
        ...
        <div class="card-info-overlay">
            ...
            <div class="card-actions">
                ...
                <button class="card-btn ${isAgotado ? 'btn-disabled' : (activeChannel === 'whatsapp' ? 'btn-wsp' : 'btn-ig')}" data-action="wsp" ${isAgotado ? 'disabled' : ''} style="display:inline-flex; align-items:center; justify-content:center; gap:4px;">
                    ${isAgotado ? 'Agotado' : (activeChannel === 'whatsapp' ? `
    ...
    const contactBtn = card.querySelector('[data-action="wsp"]');
    if (!isAgotado) {
        contactBtn.addEventListener('click', (e) => {
            ...
            if (activeChannel === 'whatsapp') {
                ...
                if (window.handleInstagramAction) {
                    window.handleInstagramAction(producto, talla, catalogConfig);
                }
            }
        });
    }
    ...
    if (hasMultipleImages) {
        let currentIdx = 0;
        const slides = card.querySelectorAll('.carrusel-slide');
        const dots = card.querySelectorAll('.carrusel-dot');
        const arrowPrev = card.querySelector('.carrusel-arrow.prev');
        const arrowNext = card.querySelector('.carrusel-arrow.next');
        ...
        const goToImage = (newIdx) => {
            ...
            dots.forEach((d, dIdx) => {
                d.classList.toggle('active', dIdx === newIdx);
                d.style.backgroundColor = dIdx === newIdx ? 'var(--z-crimson)' : 'rgba(255,255,255,0.4)';
            });
